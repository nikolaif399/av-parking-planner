...
void Tree::addEdge(int ind1, int ind2) {
  parents_[ind2] = ind1;
  edgeCosts_[ind2] = StateDistance(this->getState(ind1),this->getState(ind2));
  num_edges_++;
}
...
double Tree::getCost(int index) {
  if (states_.find(index) == states_.end()) {
    printf("Tree::getCost, Index %d not in tree\n", index);
  }
  double cost = 0;
  int cur_index = index;
  while(true) {
    double edge_cost = edgeCosts_[cur_index];
    cost += edge_cost;
    cur_index = parents_[cur_index];
    if (cur_index == 0) return cost;
  }
}
...
int Tree::getNearestVertexExcluding(State q, std::unordered_set<int> exclude) {
  int nearest_index = -1;
  double nearest_dist = std::numeric_limits<double>::max();
  for (auto const &entry : states_) {
